...
function ChannelDetailUI() {
  const maxAvatars = 5;
  const displayedAvatars = avatarItems.slice(0, maxAvatars);
  const hiddenAvatarsCount = avatarItems.length - maxAvatars;
  return (
    <>
      ...
      <div className='w-[95%] mx-auto'>
        <CategoryItems channelCategory={channelCategory} />
        <ChannelMember
          displayedAvatars={displayedAvatars}
          hiddenAvatarsCount={hiddenAvatarsCount}
        />
        <div className='font-Roboto text-white text-[20px] my-6 flex items-center gap-6'>
          <p>Videos</p>
          <PlayIcon />
          <p className='text-[15px] hover:underline'>Play all</p>
        </div>
        <CardRecommend cardData={cardData}/>
      </div>
    </>
  );
}
